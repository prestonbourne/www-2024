...
The chapter begins by claiming that many applications today are *data-intensive* as opposed to *compute-intensive*. This means bottlenecks usually manifest in data complexity, scale, and rate of change. Whilst this observation is accurate for a book written in 2017, the rise of generative AI has changed the landscape. This observation remains true for the standard CRUD app, however, with the rise of generative AI there is an increase in applications that are both data-intensive (due to the large amounts of training data required) and compute-intensive (since training involves extensive matrix multiplications and optimization processes).
...
- Faults vs Failures vs Errors
    ...
    TLDR:  In software, errors lead to faults and faults leads to failure.
    
    Though many people use the terms interchangeably, especially fault and error, there are specific differences. If there’s a faulty part of your code, the application may work fine if that part of the code doesn’t run, however if the code does run, you get an error. If your code fails to handle that error the application fails, failure is often non recoverable and the application must restart.
...
If you’re familiar with Go, you’d know that it’s crucial always handle a returned error.
